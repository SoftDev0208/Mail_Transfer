...
// ---------- TRANSPORTER (Gmail OR SMTP) ----------
// Works with BOTH:
// 1) Gmail test: set SMTP_HOST=smtp.gmail.com, SMTP_PORT=465, SMTP_SECURE=true, SMTP_USER, SMTP_PASS (App Password)
// 2) Real SMTP: set SMTP_HOST/PORT/SECURE/USER/PASS to your SMTP server
...
const transporter = nodemailer.createTransport({
    ...
    // If you are behind TLS interception and see "self-signed certificate chain", enable this:
    tls: {
...
app.get("/api/status", (req, res) => {
    db.all(
        `SELECT id, email, is_valid, is_sent, is_read
     FROM test_emails
     ORDER BY id DESC`,
        [],
        (err, rows) => {
